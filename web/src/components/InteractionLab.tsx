...
import { Terminal, Activity, Zap, ArrowRight, ShieldCheck, Clock, Sun, Lightbulb } from 'lucide-react';
  ...
  const handleInteract = async (e: React.FormEvent) => {
    ...
    try {
      if (userText.includes('timer')) {
        ...
        addTrace('action', `POST /api/v1/utility {action:'set', type:'timer', value:${seconds}, label:'${label}'}`);

        const res = await fetch(`${BACKEND}/api/v1/utility`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'set', type: 'timer', value: seconds, label }),
        });
        const data = await res.json();
        if (data.success) {
          addTrace('observation', `Timer set: ${data.expires_in}s (id: ${data.timer_id})`);
        ...
        addTrace('action', `POST /api/v1/utility {action:'query', type:'weather', label:'Vienna'}`);

        const res = await fetch(`${BACKEND}/api/v1/utility`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'query', type: 'weather', label: 'Vienna' }),
        });
        const data = await res.json();
        addTrace('observation', `${data.location}: ${data.temp}, ${data.condition}`);
        ...
        addTrace('action', `POST /api/v1/action {action_type:'light_${state}', params:{room:'living_room'}}`);

        const res = await fetch(`${BACKEND}/api/v1/action`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: `light_${state}`, params: { room: 'living_room' } }),
        });
        const data = await res.json();
        addTrace('observation', `${data.room} light → ${data.state} (${data.device})`);
        setWidgets(prev => {
          const filtered = prev.filter(w => w.type !== 'iot');
          return state === 'on'
            ? [...filtered, { id: 'iot-light', type: 'iot', label: 'Living Room', value: 'ON' }]
            : filtered;
        });

      } else {
        addTrace('thought', 'General intent. Dispatching to agentic orchestration.');
        ...
        data.trace?.forEach((step: { step: number; tool: string; status: string }) => {
          addTrace('action', `Step ${step.step}: ${step.tool} → ${step.status}`);
        });
      }

    } catch (err) {
      addTrace('error', `Backend unreachable: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  return (
    <div className="h-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-white uppercase tracking-tighter">Interaction Lab</h1>
          <p className="text-text-secondary text-sm font-bold uppercase tracking-widest opacity-60">High-fidelity domestic orchestration</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="glass-card p-10 flex flex-col min-h-[500px] shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center space-y-10 py-10 relative">
              <div className={`relative p-12 rounded-full border-2 transition-all duration-700 ${isProcessing
                ...
                <Zap className={`w-24 h-24 transition-colors duration-700 ${isProcessing ? 'text-accent-blue' : 'text-white/10'}`} />
              </div>
              <div className="space-y-3">
                <p className="text-3xl font-black text-white tracking-tighter uppercase">Substrate Active</p>
            ...
            <form onSubmit={handleInteract} className="mt-12 relative group/form">
              <input
                type="text"
                value={input}
              ...
              >
                {isProcessing ? <Activity className="w-8 h-8 animate-spin" /> : <ArrowRight className="w-8 h-8" />}
              </button>
            </form>
          </div>
          ...
          <div className="glass-card flex flex-col h-[400px] overflow-hidden shadow-xl border-white/5">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 p-6 font-mono text-sm" ref={traceEndRef}>
              {trace.length === 0 ? (
                ...
                trace.map((log) => (
                  <div key={log.id} className="flex items-start gap-4 animate-in fade-in duration-300">
                    ...
                    <span className={`px-2 py-0.5 rounded-[4px] text-xs font-black uppercase shrink-0 mt-0.5 border ${log.type === 'thought' ? 'bg-accent-blue/10 text-accent-blue border-accent-blue/20' :
                        log.type === 'action' ? 'bg-accent-purple/10 text-accent-purple border-accent-purple/20' :
                          log.type === 'observation' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                            log.type === 'error' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' :
                              'bg-white/5 text-text-secondary border-transparent'
                      }`}>
                      {log.type}
                    </span>
                    <span className={`flex-1 ${log.type === 'action' ? 'text-white font-bold' :
                        log.type === 'error' ? 'text-rose-400' :
                          log.type === 'thought' ? 'text-text-secondary italic' :
        ...
        <div className="lg:col-span-4 space-y-8">
            ...
            <div className="space-y-4">
              {widgets.length === 0 ? (
                <div className="bg-white/[0.02] border border-dashed border-white/5 rounded-2xl p-12 text-center">
                  ...
                  <p className="text-text-secondary text-xs font-black uppercase tracking-widest opacity-30">Substrate Idle</p>
                </div>
              ) : (
                widgets.map(w => (
                  <div key={w.id} className="bg-white/[0.03] border border-white/5 p-5 rounded-2xl animate-in zoom-in-95 duration-300">
                    <div className="flex items-center gap-5">
                      <div className={`${w.type === 'timer' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                          w.type === 'weather' ? 'bg-accent-blue/10 text-accent-blue border-accent-blue/20' :
                            'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                        } p-4 rounded-xl border`}>
                        {w.type === 'timer' ? <Clock size={20} /> :
                          w.type === 'weather' ? <Sun size={20} /> :
                            <Lightbulb size={20} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-[10px] font-black text-text-secondary uppercase tracking-widest opacity-40 mb-1">{w.label}</div>
          ...
          <div className="glass-card p-8 bg-accent-blue/5 border-accent-blue/10 relative overflow-hidden shadow-2xl">
            ...
            <div className="space-y-4">
              {[
                { label: 'Backend', value: 'localhost:10918', status: 'online' },
                ...
                <div key={spec.label} className="flex justify-between items-center bg-white/[0.02] p-4 rounded-xl border border-white/5">
                  <div className="text-xs font-black text-text-secondary uppercase tracking-widest opacity-40">{spec.label}</div>
                  <div className="text-sm font-black text-white tracking-widest uppercase">{spec.value}</div>
                </div>
              ))}
            </div>
          </div>
