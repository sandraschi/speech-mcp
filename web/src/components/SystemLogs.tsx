...
const SystemLogs: React.FC = () => {
    ...
    const filteredLogs = logs.filter((log: LogEntry) => {
        ...
        return [log.time, log.level, log.context, log.msg].some(s => String(s).toLowerCase().includes(q));
    });

    const exportLogs = () => {
        const text = filteredLogs.map(l => `[${l.time}] ${l.level} [${l.context}] ${l.msg}`).join('\n');
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-text-secondary text-sm mt-1 uppercase tracking-widest font-bold opacity-60">Real-time substrate telemetry</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    />
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${connected ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'} text-xs font-black uppercase tracking-widest`}>
            ...
            <div className="glass-card flex flex-col h-[600px] overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto font-mono text-sm p-2 space-y-0.5 custom-scrollbar" ref={scrollRef}>
                    {filteredLogs.length === 0 ? (
                        ...
                        filteredLogs.map((log: LogEntry, i: number) => (
                            <div key={i} className="flex gap-4 py-1 px-2 rounded hover:bg-white/[0.03] transition-colors group">
                                <span className="text-text-secondary opacity-30 select-none tabular-nums w-20 shrink-0">
                                    [{log.time}]
                                </span>
                                <span className={`font-black w-20 shrink-0 ${log.level === 'ERROR' ? 'text-rose-500' :
                                    log.level === 'WARN' ? 'text-amber-500' :
                                        log.level === 'DEBUG' ? 'text-accent-blue' : 'text-emerald-500'
                                    }`}>
                                    {log.level}
                                </span>
                                <span className="text-text-secondary opacity-40 lowercase w-24 shrink-0 truncate">[{log.context}]</span>
                                <span className="text-white opacity-80 group-hover:opacity-100 transition-opacity whitespace-pre-wrap flex-1">{log.msg}</span>
                            </div>
                        ))
                    )}
                </div>
