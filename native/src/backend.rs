...
const BACKEND_PORT: u16 = 10909;
const BACKEND_TAG: &str = "speech-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "SPEECH_MCP_PORT";
const ENV_TAURI: &str = "SPEECH_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'speech-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'speech-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM speech-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM speech-mcp-native.exe /T 2>$null"
        );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    cmd.args(["--http", "--port", &BACKEND_PORT.to_string()])
